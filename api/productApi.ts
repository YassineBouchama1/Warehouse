...
export const updateProductDetails = async (productId: number, updatedDetails: Partial<Product>): Promise<Product> => {
  const product = await fetchProductById(productId)
...
export const addStockToProduct = async (productId: number, warehouseId: number, quantity: number,city:string): Promise<Product> => {
  const product = await fetchProductById(productId)
